...
// Browser-side weather enrichment. Mirrors the parsing in
// `@/lib/weather.ts` (server-side, no longer used by /api/courts since live
// slots arrive client-side). Kept separate so this module stays free of
// server-only fetch options. Open-Meteo allows browser CORS (`*`).
...
/** Attach hourly forecast weather to every slot; never throws. */
export async function enrichCourtsWithWeatherClient(
  ...
  for (let i = 0; i < locations.length; i += WEATHER_BATCH_SIZE) {
    ...
    const results = await Promise.all(
      batch.map(async (location) => ({
        ...
        weather: await fetchLocationWeather(location.lat, location.lng),
      }))
    );
...
async function fetchLocationWeather(lat: number, lng: number): Promise<Map<string, SlotWeather>> {
  try {
    ...
    for (let i = 0; i < times.length; i++) {
      const [date, hhmm] = times[i].split("T"); // 2026-04-11T08:00
      out.set(normalizeSlotKey(date, hhmm.slice(0, 5)), formatWeather(temps[i], precip[i], codes[i], wind[i]));
    }
    return out;
  } catch {
    return new Map();
...
function normalizeSlotKey(date: string, time: string): string {
  ...
  const nextDate = normalizedDate.toLocaleDateString("en-CA", { timeZone: "America/Los_Angeles" });
...
function describeWeatherCode(code: number | null): { label: string; emoji: string } {
  switch (code) {
    case 0: return { label: "Clear", emoji: "☀️" };
    case 1:
    case 2: return { label: "Partly cloudy", emoji: "⛅" };
    case 3: return { label: "Cloudy", emoji: "☁️" };
    case 45:
    case 48: return { label: "Fog", emoji: "🌫️" };
    case 51:
    case 53:
    case 55:
    case 56:
    case 57: return { label: "Drizzle", emoji: "🌦️" };
    case 61:
    case 63:
    case 65:
    case 66:
    ...
    case 80:
    case 81:
    case 82: return { label: "Rain", emoji: "🌧️" };
    case 71:
    case 73:
    case 75:
    case 77:
    case 85:
    case 86: return { label: "Snow", emoji: "🌨️" };
    case 95:
    case 96:
    case 99: return { label: "Storm", emoji: "⛈️" };
    default: return { label: "Forecast", emoji: "🌤️" };
  }
}
